...
| 14 | 证据是真的。<br>你：可能只是人好。<br>NED：很好，你已经会用了。👍 | `self_discount_positive` 屏 Extreme **canonical unit（三行不可拆分）** |

> **#14 是不可拆分的三行单位**，拆开笑点即垮。第一行固定为「证据是真的。」——**不得**改成「她都说爱你了。」，
> 因为该 situation 覆盖的不只是 declaration 类证据（例如"她主动找我聊了两个小时，可能只是人好"），
> 改成"她都说爱你了"会让第一屏说错事实。三行中的 `NED：很好，你已经会用了。👍` 也可单独复用于
> 其它"用户替 NED 干活"的场景；`驳回理由已由申请人自行填写。👍` 同理。
